...
opt =
  Opt
    <$> some (argument str (metavar "TAGS..."))
    <*> option
    ...
    <*> option
          ...
          (  long "delay"
          <> short 'd'
          <> help "Start next try after DELAY ms"
          <> showDefault
          <> value 100
          <> metavar "DELAY"
          )
 ...
 where
  ...
  checkWithin sets v | v `elem` sets = Right v
                     | otherwise     = Left $ "Unknown parameter: " ++ v

opts :: ParserInfo Opt
opts = info
  (opt <**> helper)
  (fullDesc <> progDesc "Download all images about TAGS" <> header
    ("kona " ++ showVersion version ++ " – A Crawler for Konachan.com")
  )

parseOpts :: Opt -> CrawlerConfig
parseOpts (Opt tag rtg knd ord workers out delay retries limit exclusion) =
  CrawlerConfig (mkParams [tags (rating rtg : order ord : tag)])
                workers
                limit
                exclusion
